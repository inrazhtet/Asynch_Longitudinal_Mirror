...
**[UPDATE]** The singletons will **NOT** be removed from the file. Instead, LOCF and LOCB will be applied. This will be done once, the interpolation function has been tested.
...
**[EDIT]** Need to document how many matches are in each data set and both data sets. **[EDIT]**
...
This is where we join by the patient ID to only those subjects that exist in both datasets. 

*Step 1: Find all the Common IDs that match
*Step 2: Extract only those IDs from each table
*Step 3: For each ID, expand by the row number by the total number of unique time intervals.
*Step 4: For BMI, add in those missing time intervals, (sort in order) and expand for each subject ID.
*Step 5: For Media, add in those missing time intervals,(sort in order) and expand for each subject ID.
...
length(common_ID) #436 common subject IDs
```
...
#View(c_data_arr_mer)
write.csv(c_data_arr_mer, "../../data/processing/merged_arranged_data.csv")
```


*Step 7: Split the data set by the Subject ID *
...
c_data_split <- split(c_data_arr_mer, c_data_arr_mer[,1])
```

*Step 8: Build Custom Function to Handle Interpolation*


```{r}
#The function will take in a data frame as well as an input vector that specifies which column indexes of the data frame are of interest for the interpolation
mdz_interpolate <- function(x, par){
...
a) We can check Daphna's lapply to see how this can be worked out




b) We can check dplyr functional apply and how parameters are fitted
...
mdz_interpolate <- function(df, par){
  ...
  x <- df
  print(x)
  #Pulling out the index for X vector
  x_1 <- par[1]
  #Pulling out the index for Y Vector 1
  y_1 <- par[2]
  #Pulling out the index for Y Vector 2
  #y_2 <- par[3]
  #Pulling out x and y vectors for the interpolation
  xx <- x[,x_1] # X vector
  yy_1 <- x[,y_1] # Y vector 1
  #yy_2 <- x[,y_2] # Y vector 2

  #Specifying indexes where we had fill the missing NA
  xout_1 <- which(is.na(yy_1))
  browser()
  #Specifying the minimum and maximum values for Last Value Carried Backward/Forward
  y_nmis_1 <- which(!is.na(yy_1))# Get all the non missing y values
  y_min_1 <- yy_1[min(y_nmis_1)] # Get the value from the furthest left index of Y
  y_max_1 <- yy_1[max(y_nmis_1)] # Get the value from the furthest right index ofY
...
Testing with 1 data frame

```{r}
test_1 <- c_data_split$`1`
test_output <- mdz_interpolate(test_1, c(2,3))
