...
#apply the linear interpolation through the wrapper mdz_interpolate which utilizes approx function
combined_data_interpolated <- lapply(combined_data_split, function(x) mdz_interpolate(x,"Months","Media","linear",2)) 
```

###### Step 7: Combining the interpolated data set and saving it

The imputed but split data is combined back to into a single data frame and saved as a data set as the final step.
