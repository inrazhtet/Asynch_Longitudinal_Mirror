...
sum(is.na(bmi_media_raw))
```

Removing all the missing values of bmi_media_raw
```{r}
bmi_media_raw_nna <- bmi_media_raw[complete.cases(bmi_media_raw),] #Getting only the complete rows
print(sum(is.na(bmi_media_raw_nna)))
```
...
```{r}
#The base data set for all the Ids for this Lowess Curve
base <- bmi_media_raw_nna[bmi_media_raw_nna$Months >=5.5 & bmi_media_raw_nna$Months <= 6.5,]
sum(is.na(base))
#The distribution of the cut off points for media usage
print(quantile(base$Media))
```
...
base_6 <- base_alltime[base_alltime$Months %in% c(6),]
base6_l1 <- base_6[base_6$Media >=0 & base_6$Media <4.11, ]
base6_l2 <- base_6[base_6$Media >=4.11 & base_6$Media <4.8, ]
base6_l3 <- base_6[base_6$Media >=4.8 & base_6$Media <5.29, ]
base6_l4 <- base_6[base_6$Media >=5.29 & base_6$Media <6.38, ]
```
...
plot(x, y, type = "n", main = "4 Bucket BMI Trajectory with Raw Data", ylim = c(-0.5,1.5))
lines(loess.smooth(x = base_l1$Months, y = base_l1$zBMI), col = "green")
lines(loess.smooth(x = base_l2$Months, y = base_l2$zBMI), col = "yellow")
lines(loess.smooth(x = base_l3$Months, y = base_l3$zBMI), col = "orange")
lines(loess.smooth(x = base_l4$Months, y = base_l4$zBMI), col = "blue")
